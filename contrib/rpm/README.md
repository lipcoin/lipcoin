...
The RPM spec file provided here is for LipCoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://lipcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[LipCoin git repository](https://github.com/lipcoin/lipcoin) but are not part of
...
Currently only `Source10` is used, for the example `lipcoin.conf` file.
...
in the lipcoin git repository. At present only an SVG version of the LipCoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the lipcoin
...
needed when 0.12.1 is released, a proper fix is already in the LipCoin git
master branch.
...
idea to build LipCoin Core against a static version of that library compiled
...
The `lipcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `lipcoin-server` package. At this point, most RPM based
...
The `lipcoin-server` package should have SELinux support. How to properly do
...
LipCoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/lipcoin/). He has been
packaging LipCoin for Fedora at least since 2012.
...
4. I renamed the `lipcoin` package that contains the Qt GUI to `lipcoin-core` as
that appears to be how the general population refers to it, in contrast to
`lipcoin-xt` or `lipcoin-classic`. I wanted to make sure the general population
