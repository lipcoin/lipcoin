...
###   This script attempts to download the signature file SHA256SUMS.asc from lipcoin.org
...
WORKINGDIR="/tmp/lipcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
BASEDIR="https://lipcoin.org/bin/"
VERSIONPREFIX="lipcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'lipcoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the LipCoin Core binary release signing key installed?"
   fi
