#!/bin/bash
# Copyright (c) 2013 The LipCoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete LipCoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no LipCoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old LipCoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected LipCoin 0.7 datadir."
    ;;
  3)
    echo "Detected LipCoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected LipCoin 0.8 datadir."
    ;;
esac
