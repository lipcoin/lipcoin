...
#if defined(NDEBUG)
# error "LipCoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "LipCoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s LIPC additional fees, %d delta bytes\n",
...
CAmount GetLIPCSubsidy(int nHeight, const Consensus::Params& consensusParams) {
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
	...
	if(nHeight < consensusParams.nDiffChangeTarget) {
		...
		nSubsidy = GetLIPCSubsidy(nHeight, consensusParams);
	}

	//make sure the reward is at least 1 LIPC
	if(nSubsidy < COIN) {
		nSubsidy = COIN;
	}
